...
def get_train_valid_loader_consensus(csv, batch_size=16,
                                    ...
                                    num_workers=4,
                                    pin_memory=False,
                                    mode='ShuffleSplit', nb_loaders=5, strong_test=False, test_size=0.0001,
                                    valid_size=0.2):
    """
    ...
    if mode == 'KFold':
        ...
        kf = ShuffleSplit(n_splits=nb_loaders, test_size=test_size, train_size=None, random_state=random_seed)
    else:
...
shuffle_mode = 'ShuffleSplit'
test_size = 0.0001
valid_size = 0.2


trainloaders = get_train_valid_loader_consensus('../Data/Crohn2020_consensus/consensus_full.csv', batch_size=16,
                                                    random_seed=88,
                                                    subset='train', nb_loaders=5,
                                                    num_workers=0,
                                                    mode=shuffle_mode, test_size=test_size, valid_size=valid_size)


validloaders = get_train_valid_loader_consensus('../Data/Crohn2020_consensus/consensus_full.csv', batch_size=16,
                                                    random_seed=88,
                                                    subset='test', nb_loaders=5,
                                                    num_workers=0,
                                                    mode=shuffle_mode, test_size=test_size, valid_size=valid_size)
