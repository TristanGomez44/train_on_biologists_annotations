...
class ResNet(nn.Module):
    ...
    def __init__(self, block,layers,num_classes=1000,zero_init_residual=False,norm_layer=None,stride=2,\
                    strideLay2=2,strideLay3=2,strideLay4=2,chan=64,inChan=3):
        ...
        self.layer1 = self._make_layer(block, chan[0], layers[0], stride=1,norm_layer=norm_layer)
        self.layer2 = self._make_layer(block, chan[1], layers[1], stride=strideLay2, norm_layer=norm_layer)
        self.layer3 = self._make_layer(block, chan[2], layers[2], stride=strideLay3, norm_layer=norm_layer)
        self.layer4 = self._make_layer(block, chan[3], layers[3], stride=strideLay4, norm_layer=norm_layer)
