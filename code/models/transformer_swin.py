...
class SwinTransformerV2(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x_pooled = self.avgpool(x)  # B C 1
        x_pooled = torch.flatten(x_pooled, 1)
        return x_pooled,x

    def forward(self, x):
        feat_pooled,feat = self.forward_features(x)
        #x = self.head(x)
        size = int(sqrt(feat.shape[2]))
        feat = feat.view(feat.shape[0],feat.shape[1],size,size)
        return {"feat_pooled":feat_pooled,"feat":feat}
