...
def resnet50(pretrained=False,fullpretrained=False, **kwargs):
    """Constructs a ResNet-50 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(Bottleneck, [3, 4, 6, 3], **kwargs)
    print(pretrained,fullpretrained)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet50']))
    if fullpretrained:
        weights= torch.load("./models/abn.pth")["state_dict"]

        weights = {key.replace("module.",""):weights[key] for key in weights}
